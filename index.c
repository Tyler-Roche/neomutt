...
/**
 * struct SelectedEmail - keep track of the currently selected email.
 */
 ...
 * @param curr  Currently selected email
 * @param e     Email to check
 * @retval true e is curr
 * @retval false e is not curr
 */
static bool is_current_email(const struct SelectedEmail *curr, const struct Email *e)
{
  return (e->received == curr->received) &&
         mutt_str_equal(e->env->message_id, curr->message_id);
}
 ...
 */
static void set_current_email(struct SelectedEmail *curr, const struct Email *e)
{
  *curr = (struct SelectedEmail){
    .received = e ? e->received : 0,
    .message_id = mutt_str_replace(&curr->message_id, e ? e->env->message_id : NULL)
  };
}
...
static void update_index(struct Menu *menu, struct Context *ctx, int check,
                         int oldcount, const struct SelectedEmail *curr)
{
  if (!menu || !ctx)
    return;
 ...
 * @param check      Flags, e.g. #MUTT_REOPENED
 * @param oldcount   How many items are currently in the index
 * @param curr_email Currently selected email
 * @note curr_email can not be NULL.
 */
void mutt_update_index(struct Menu *menu, struct Context *ctx, int check,
                       int oldcount, const struct Email *curr_email)
{
  struct SelectedEmail se = { .received = curr_email->received,
                              .message_id = curr_email->env->message_id };
 ...
 * @param m          Mailbox
 * @param oldcount   How many items are currently in the index
 * @param curr       Remember our place in the index
...
static void change_folder_mailbox(struct Menu *menu, struct Mailbox *m, int *oldcount,
                                  const struct SelectedEmail *curr, bool read_only)
{
  if (!m)
    return;
    ...
    *oldcount = Context->mailbox->msg_count;
    ...
    if (check != 0)
    {
      ...
      if ((check == MUTT_NEW_MAIL) || (check == MUTT_REOPENED))
        update_index(menu, Context, check, *oldcount, curr);
                                             ...
                                             const struct SelectedEmail *curr, bool read_only)
{
  ...
  change_folder_mailbox(menu, m_query, oldcount, curr, read_only);
  return m_query;
}
#endif
                                 ...
                                 bool *pager_return, bool read_only)
{
  ...
  if ((type == MUTT_MAILBOX_ERROR) || (type == MUTT_UNKNOWN))
  {
    ...
    if (m)
    {
      change_folder_mailbox(menu, m, oldcount, curr, read_only);
      *pager_return = false;
    }
    else
    ...
    switch (op)
    {
      ...
      case OP_MAIN_SYNC_FOLDER:
          ...
          break;
        {
          ...
          if (check == 0)
          {
            ...
            update_index(menu, Context, check, oc, &curr);
      ...
      case OP_MAIN_ENTIRE_THREAD:
      {
        ...
        if (Context->mailbox->type != MUTT_NOTMUCH)
        {
          ...
          change_folder_notmuch(menu, buf, sizeof(buf), &oldcount, &curr, false);
      ...
      case OP_MAIN_VFOLDER_FROM_QUERY_READONLY:
      {
        ...
        struct Mailbox *m_query =
            change_folder_notmuch(menu, buf, sizeof(buf), &oldcount, &curr,
      ...
      case OP_SIDEBAR_OPEN:
        change_folder_mailbox(menu, sb_get_highlight(), &oldcount, &curr, false);
        break;
#endif
      ...
      case OP_MAIN_NEXT_UNREAD_MAILBOX:
      {
        ...
        change_folder_mailbox(menu, m, &oldcount, &curr, false);
        break;
      }
...
#endif
      {
        ...
        else
        {
          change_folder_string(menu, folderbuf->data, folderbuf->dsize,
                               &oldcount, &curr, &pager_return, read_only);
        }
      ...
      case OP_DISPLAY_HEADERS: /* don't weed the headers */
      {
        ...
        menu->oldcurrent = menu->current;
        if (Context && Context->mailbox)
          update_index(menu, Context, MUTT_NEW_MAIL, Context->mailbox->msg_count, &curr);
        continue;
      }
